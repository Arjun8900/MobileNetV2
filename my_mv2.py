...
from keras.layers import DepthwiseConv2D, ReLU, Dense
...
def B1(inputs, kernel, filters, t,s):
    ...
    x = Conv2D(in_conv2d, (1,1), padding='same', strides=(1,1))(inputs)   # Pointwise(Whenever kernel=(1,1) in Conv2D it is Pointwise)
    ...
    if s==1:
        ...
        pad = 'valid'
    x = DepthwiseConv2D(kernel, strides=(s,s), depth_multiplier=1, padding='same')(x)    # For DWC2D, kernel=(3,3) always,  Only s changes
    x = BatchNormalization(axis=-1, momentum=0.999)(x)
    x = keras.layers.ReLU(6.)(x)
    
    #x = Activation('relu')(x)
    
    x = Conv2D( out_conv2d, (1,1), strides=(1,1), padding='same')(x)   # Pointwise(Whenever kernel=(1,1) in Conv2D it is Pointwise)
    x = BatchNormalization(axis=-1, momentum=0.999)(x)
    return x
...
def B2(inputs, kernel, filters, t,s):
    ...
    x = Conv2D(in_conv2d, (1,1), padding='same', strides=(1,1))(inputs)   # Point wise (Whenever kernel=(1,1) in Conv2D it is Pointwise)
    x = BatchNormalization(axis=-1, momentum=0.999)(x)
    x = keras.layers.ReLU(6.)(x)
    ...
    x = Conv2D(out_conv2d, (1,1), strides=(1,1), padding='same')(x)   # Point wise (Whenever kernel=(1,1) in Conv2D it is Pointwise)
    x = BatchNormalization(axis=-1, momentum=0.999)(x)
...
def My_Mobilenetv2(input_shape, num_classes):
    ...
    x = Conv2D(32, (3,3), padding='same', strides = (2,2))(inputs)    # NOT POINTWISE
    x = BatchNormalization(axis=-1, momentum=0.999)(x)
    x = keras.layers.ReLU(6.)(x)
    ...
    x = Conv2D(1280, (1,1), padding='same', strides=(1,1))(x)   #Pointwise (Whenever kernel=(1,1) in Conv2D it is Pointwise)
    x = BatchNormalization(axis=-1, momentum=0.999)(x)
    x = keras.layers.ReLU(6.)(x)
